...
def shortest_path(source, target):
    ...
    while True:
        ...
        # If no solution exist, return None
        if frontier.empty():
            return None
        ...
        for movie, actor in neighbors_for_person(node.state):
            # ignore actors that are in the frontier and explored set
            if not frontier.contains_state(actor) and actor not in explored:
                child = Node(state=actor, parent=node, action=movie)

                # if the child is the target, return the solution
                if child.state == target:
                    solution = []
                    while child.parent is not None:
                        solution.append((child.action, child.state))
                        child = child.parent
                    # Reverse to get initial state to goal
                    solution.reverse()
                    return solution
                else:
                    # Or else add it to the frontier
                    frontier.add(child)
